...
ELSE()
  ...
  ENDIF()
  SET(CHROMIUM_PLAT_LDFLAGS ${CHROMIUM_PLAT_LDFLAGS}  smime3 plds4 plc4 pthread gdk-x11-2.0 gdk_pixbuf-2.0 pangocairo-1.0 gio-2.0 pango-1.0 cairo gobject-2.0 gmodule-2.0 glib-2.0 fontconfig cups freetype rt gconf-2 glib-2.0 dbus-glib-1 X11 Xtst asound harfbuzz sandbox)
...
SET(CHROMIUMLIBS ${CHROMIUMLDFLAGS} ${CHROMIUM_TPLIBS} gfx dl m common common_net dynamic_annotations browser debugger renderer utility printing app_base appcache base base_i18n protobuf_lite glue icui18n installer_util database syncapi sync icuuc icudata skia skia_opts xml2 net net_base cld ots googleurl sdch modp_b64 v8_snapshot v8_base pcre wtf webkit fileapi webcore_svg webcore_bindings webcore_remaining webcore_platform webcore_html webkit_user_agent blob speex media ffmpeg http_server cacheinvalidation chromoting_base chromoting_client chromoting_host chromoting_protocol chromoting_jingle_glue chromotocol_proto_lib trace_proto_lib jingle_p2p chromoting_plugin notifier ppapi_cpp_objects ppapi_proxy ppapi_shared_impl sync_notifier gpu_plugin gles2_implementation gles2_c_lib command_buffer_client command_buffer_service command_buffer_common gles2_cmd_helper chrome_gpu hunspell plugin ipc worker common_constants default_plugin jingle translator_common translator_glsl profile_import service policy webp flac gpu_ipc yuv_convert_sse2 yuv_convert yarr i18n_api cpu_features webkit_gpu)
IF (APPLE OR LINUX_ENABLE_NACL)
  SET(CHROMIUMLIBS ${CHROMIUMLIBS} nacl nacl_base google_nacl_npruntime nonnacl_srpc sel sel_ldr_launcher nonnacl_util_chrome gio gio_shm platform nrd_xfer expiration debug_stub platform_qual_lib)
ENDIF()
